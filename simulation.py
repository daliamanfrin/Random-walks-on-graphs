...
collection_time = config.getint('settings', 'collection_time')
...
if dynamics_type == 'synchronous':
    particle_counts = random_walk.synchronous_simulation(network, n_max, time_steps, collection_time)
elif dynamics_type == 'one_step':
    particle_counts = random_walk.one_step_process(network, n_max, time_steps, collection_time)
else:
